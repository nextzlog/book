...
prefs = dict(np.loadtxt('pref%i.dat' % int(sys.argv[1]), delimiter=',', dtype='str'))
...
for geom,pref in map(lambda p:(p.geometry,p.attributes['name']), japan):
	ax.add_geometries(geom, crs=ccrs.PlateCarree(), facecolor=colors[int(prefs[pref])], edgecolor='black', lw=1)
ax.add_geometries(north[0].geometry, crs=ccrs.PlateCarree(), facecolor=colors[int(prefs['Hokkaido'])], edgecolor='black', lw=1)
