...
plt.ylabel("")
plt.grid(ls='dotted')
plt.savefig('plot.{}.svg'.format(ID), bbox_inches='tight', pad_inches=0.1)
plt.savefig('plot.{}.eps'.format(ID), bbox_inches='tight', pad_inches=0.1)
os.remove('class.dat')
os.remove('data0.dat')
os.remove('data1.dat')
os.remove('data2.dat')
webbrowser.open('file://{}'.format(os.path.realpath('plot.{}.svg'.format(ID))))
